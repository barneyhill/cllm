#!/usr/bin/env python3
"""
Convert JSON data to a professional landscape PDF table with text wrapping.
Handles columns with lots of text by wrapping content within cells.
"""
...
def json_to_pdf_table(json_data, output_filename, title=None):
    ...
    long_text_columns = {'notes', 'llm_reasoning', 'peer_reasoning', 'reasoning'}
    ...
    for header in headers:
        ...
        if header_lower in ['llm_reasoning', 'peer_reasoning', 'reasoning']:
        ...
        elif header_lower in ['llm_status', 'peer_status', 'agreement_status']:
...
def main():
    ...
    # Example JSON data (same as your example)
    sample_data = [
        {
            "llm_result_id": "R7",
            "peer_result_id": "R7",
        ...
        },
        {
            ...
            "agreement_status": "agree",
            "notes": "Both assess technical implementation as SUPPORTED with evidence from the manuscript.",
            "n_llm": 8,
            "n_peer": 5,
            "n_itx": 5,
            "llm_reasoning": "The technical implementation is well-documented in the methods section with code examples and validation results.",
            "peer_reasoning": "Reviewers confirmed the technical approach is sound and reproducible based on the provided documentation."
        },
        {
            "llm_result_id": "R3",
            "peer_result_id": "R3",
            ...
            "notes": "LLM finds partial support while peer reviewers remain uncertain about generalizability claims.",
            "n_llm": 12,
            "n_peer": 8,
            "n_itx": 4,
            "llm_reasoning": "The claims about generalizability are supported by some experiments but lack comprehensive validation across different datasets and conditions. The evidence provided covers common use cases but doesn't fully address edge cases mentioned in the discussion section.",
            "peer_reasoning": "Reviewer 3 notes that while the approach shows promise, the generalizability claims need more extensive validation. The current evidence is limited to specific experimental conditions and may not apply broadly without additional testing."
        }
    ]
    
    # Create PDF from sample data
    json_to_pdf_table(
        sample_data,
        "/mnt/user-data/outputs/json_table_output.pdf",
        title="Results Comparison Table"
    )
    
    # You can also load from a JSON file:
    # json_to_pdf_table("data.json", "output.pdf", title="My Data Table")
